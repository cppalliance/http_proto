...
public:
        ...
        header const& h)
    {
        ...
        prepped_.append({ h.h_.cbuf, h.h_.size });
        more_input_ = false;
    }
...
serializer::
start_stream(
    header const& h) -> stream
{
    BOOST_ASSERT(impl_);
    return impl_->start_stream(h);
}
